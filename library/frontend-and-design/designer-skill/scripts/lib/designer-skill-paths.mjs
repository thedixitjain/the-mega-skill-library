...
export const DESIGNER_SKILL_DIR = '.designer-skill';
export const LIVE_DIR = 'live';
export const CRITIQUE_DIR = 'critique';

export function getDesignerSkillDir(cwd = process.cwd()) {
  return path.join(cwd, DESIGNER_SKILL_DIR);
}

export function getDesignSidecarPath(cwd = process.cwd()) {
  return path.join(getDesignerSkillDir(cwd), 'design.json');
}
...
export function getLiveDir(cwd = process.cwd()) {
  return path.join(getDesignerSkillDir(cwd), LIVE_DIR);
}
...
export function resolveLiveConfigPath({ cwd = process.cwd(), scriptsDir, env = process.env } = {}) {
  if (env.DESIGNER_SKILL_LIVE_CONFIG && env.DESIGNER_SKILL_LIVE_CONFIG.trim()) {
    const configured = env.DESIGNER_SKILL_LIVE_CONFIG.trim();
...
export function getLegacyLiveServerPath(cwd = process.cwd()) {
  return path.join(cwd, '.designer-skill-live.json');
}
...
export function getLegacyLiveSessionsDir(cwd = process.cwd()) {
  return path.join(cwd, '.designer-skill-live', 'sessions');
}
...
export function getCritiqueDir(cwd = process.cwd()) {
  return path.join(getDesignerSkillDir(cwd), CRITIQUE_DIR);
}

export function getLegacyLiveAnnotationsDir(cwd = process.cwd()) {
  return path.join(cwd, '.designer-skill-live', 'annotations');
}
