...
<body>
    <div class="deck-viewport">
        <main class="deck-stage" id="deckStage">
            ...
            <!-- Presentation chrome (controls, counter, nav dots) goes HERE:
                 inside .deck-stage, AFTER the slides, sized for the 1920×1080
                 canvas (e.g. counter ~20px, buttons ~23px). It scales with the
                 stage — chrome outside the stage does not scale in iframes and
                 overlaps content when the deck shrinks. -->
            <div class="deck-controls"><!-- prev/next, progress, counter --></div>
        </main>
    </div>
    ...
    <script>
        ...
        class SlidePresentation {
            ...
            /* Dual-mode scaler — REQUIRED shape (embed-safe + standalone-safe).
               Standalone: min-fit the window; body flex centers the viewport.
               Embedded (iframe): scale by WIDTH ONLY so the document takes a
               natural 16:9 height (an auto-height host like the Confluence
               HTML macro grows to fit) and resize a same-origin fixed-height
               host iframe directly. Never use translate() for centering and
               never compute embed sizes from vh/% — auto-height iframes
               report ~0 and lock the collapsed height in. */
            setupStageScale() {
                ...
                const scale = () => {
                    /* clientWidth/Height, not innerWidth: immune to scrollbar/headless quirks */
                    const vw = document.documentElement.clientWidth || window.innerWidth;
                    if (embedded) {
                        const f = vw / 1920;
                        viewport.style.width = (1920 * f) + 'px';
                        viewport.style.height = (1080 * f) + 'px';
                        this.stage.style.transform = `scale(${f})`;
                        try {
                            if (window.frameElement) window.frameElement.style.height = (1080 * f) + 'px';
                        } catch (e) { /* cross-origin host: auto-height or a 16:9 macro height covers it */ }
                        return;
                    }
                    const vh = document.documentElement.clientHeight || window.innerHeight;
                    const f = Math.min(vw / 1920, vh / 1080);
                    viewport.style.width = (1920 * f) + 'px';
                    viewport.style.height = (1080 * f) + 'px';
                    this.stage.style.transform = `scale(${f})`;
                };
...
2. **Stage Scaling (dual-mode, embed-safe)** — For fixed 16:9 presentation behavior:
   - Keep all slides at 1920×1080 inside `.deck-stage`
   - Scale the whole stage with one `scale()` transform; center via body flex, not `translate()`
   - Letterbox/pillarbox as needed; never reflow slide content per device
   - Detect iframes (`window.self !== window.top` in try/catch), add `.embed` to `<html>`, and scale by width only so the document has a natural 16:9 height; set `window.frameElement.style.height` in a try/catch for same-origin hosts
   - Keep all presentation chrome inside `.deck-stage` so it scales too
