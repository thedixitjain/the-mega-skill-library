---
name: claude-quickstarts-claude
description: "Long-running Node server (Hono): browser chat with a sessions sidebar (useChat → /api/chat) → Chat SDK web adapter onDirectMessage → one Managed Agents session per conversation, where the conversation ID is the session ID (no mapping, no database; the sidebar is sessions.list, transcript replay is sessions.events.list) → a held session event stream (opened with eventdeltas token previews, always on) feeds each fragment back onto the same HTTP response while /api/activity streams the tool calls and thinking the page shows as a live feed. Each research turn closes with a kept tool-call trace (a"
category: ai-agents-and-harness
source_repo: anthropics/claude-quickstarts
source_path: "managed-agents/chat-sdk/CLAUDE.md"
source_url: https://github.com/anthropics/claude-quickstarts/blob/HEAD/managed-agents/chat-sdk/CLAUDE.md
---
...
**Source:** [`anthropics/claude-quickstarts`](https://github.com/anthropics/claude-quickstarts) → `managed-agents/chat-sdk/CLAUDE.md`
